...
function seedIfEmpty() {
  const count = db.prepare("SELECT COUNT(*) AS count FROM coaches").get().count;
  if (count > 0) return;

  const coachId = uid("coach");
  const teamId = uid("team");
  db.prepare('INSERT INTO coaches (id, name, email, password, "PW_Recovery_Key", team_name) VALUES (?, ?, ?, ?, ?, ?)').run(
    coachId,
    "Coach Carter",
    "coach@northview.org",
  ...
  db.prepare(
    "INSERT INTO teams (id, coach_id, name, location, sport) VALUES (?, ?, ?, ?, ?)"
  ).run(teamId, coachId, "Northview Panthers", "Albany, NY", "football");

  const demoPlayers = [
    ["Jordan", "Reed", "jordan@example.com"],
    ["Tyler", "Shaw", "tyler@example.com"],
    ["Evan", "Brooks", "evan@example.com"]
  ];

  for (const [first, last, email] of demoPlayers) {
    const playerId = uid("player");
    const publicId = playerPublicId();
    db.prepare(
      `INSERT INTO players
      (id, team_id, first_name, last_name, email, team_name, player_public_id, password, "PW_Recovery_Key", registered, published)
      VALUES (?, ?, ?, ?, ?, ?, ?, '', ?, 0, 0)`
    ).run(playerId, teamId, first, last, email, "Northview Panthers", publicId, recoveryKey());

    const template = equipmentTemplateForSport("football");
    template.forEach((row, index) => {
      db.prepare(
        `INSERT INTO equipment_items
        (id, player_id, name, category, price_range, goal, raised, enabled, sort_order)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)`
      ).run(uid("eq"), playerId, row[0], row[1], row[2], index === 0 ? 150 : 50, index === 0 ? 60 : 15, index);
    });
  }
}
